...
from .blender_re_clsp import importCLSPFile
    ...
    def execute(self, context):
        ...
        if chainCollection != None and headerObj != None:
            ...
            if not valid:
                ...
                chainEntryCollection = getCollection(f"Chain Groups - {chainCollection.name}",chainCollection,makeNew = False)
...
class WM_OT_CreateFullBodyClsp(Operator):
    bl_label = "Create Full Body Clsp"
    bl_idname = "re_chain.create_full_body_clsp"
    bl_options = {'UNDO'}
    bl_description = ""
    ...
    def execute(self, context):
        ...
        if chainCollection != None and (headerObj != None or isCLSP):
            ...
            if not valid:
                ...
                collisionCollection = getCollection(f"Chain Clsps - {clspname}.clsp",chainCollection,False)
...
class WM_OT_NewChainHeader(Operator):
    ...
    # 	items=[ (".chain2", "Chain2", "New chain file format, use for MH Wilds/Dead Rising and newer"),
    ...
    def execute(self, context):
        ...
        if self.collectionName.strip() != "":
            ...
            chainCollection = createChainCollection(self.collectionName.strip() + ".chain2",parentCollection)
            # if self.chainFormat != ".clsp":
            chainHeaderObj = createEmpty(f"CHAIN_HEADER {self.collectionName}.chain2", [("TYPE","RE_CHAIN_HEADER")],None,chainCollection)
...
class WM_OT_NewChainSettings(Operator):
    ...
    def execute(self, context):
        ...
        if chainCollection != None and headerObj != None:
            ...
            chainEntryCollection = getCollection(f"Chain Groups - {chainCollection.name}",chainCollection,False)
            # chainEntryCollection = chainCollection
            currentIndex = 0

            subname = "CHAIN_SETTINGS_" + str(currentIndex).zfill(2)
            while(checkNameUsage(subname,checkSubString=True)):
                currentIndex +=1
                subname = "CHAIN_SETTINGS_" + str(currentIndex).zfill(2)

            settingname = subname + "_" + chainList[len(chainList) - 1].name.rsplit("_", 1)[0]
            ...
            chainSettingsObj = createEmpty(settingname, [("TYPE","RE_CHAIN_CHAINSETTINGS")],headerObj,chainEntryCollection)
...
class WM_OT_NewChainJiggle(Operator):
    ...
    def execute(self, context):
        ...
        if chainCollection != None:
            ...
            chainJiggleObj = createEmpty(name, [("TYPE","RE_CHAIN_JIGGLE")],parent,parent_collection)
...
class WM_OT_CreateChainLinkCollision(Operator):
    bl_label = "Create Link Collision"
...
class WM_OT_ApplyChainPreset(Operator):
    bl_label = "Apply Chain Preset"
    bl_idname = "re_chain.apply_chain_preset"
    bl_description = "Apply preset to selected chain objects. Note that frame orientations are not changed by presets"

    def execute(self, context):
        finished = False
        enumValue = bpy.context.scene.re_chain_toolpanel.chainPresets
        ...
        if finished:
            self.report({"INFO"}, "Applied chain preset.")
            return {'FINISHED'}
        else:
            return {'CANCELLED'}
...
class WM_OT_OpenPresetFolder(Operator):
    ...
    def execute(self, context):
        presetsPath = os.path.join(os.path.dirname(os.path.split(os.path.abspath(__file__))[0]),"Presets/ChainPresets")
...
class WM_OT_AlignBoneTailsToAxis(Operator):
    ...
    def poll(self,context):
        return context.active_object is not None
    def execute(self, context):
        toolpanel = bpy.context.scene.re_chain_toolpanel
        selected = bpy.context.selected_pose_bones
        chainList = []
        if len(selected) == 1:
            startBone = selected[0]
        ...
        if toolpanel.posebonealignAxis == "X":
            alignVector = Vector((1.0,0.0,0.0))
        elif toolpanel.posebonealignAxis == "-X":
            alignVector = Vector((-1.0,0.0,0.0))
        elif toolpanel.posebonealignAxis == "Y":
            alignVector = Vector((0.0,1.0,0.0))
        elif toolpanel.posebonealignAxis == "-Y":
            alignVector = Vector((0.0,-1.0,0.0))
        elif toolpanel.posebonealignAxis == "Z":
...
class WM_OT_CreateChainSubGroup(Operator):
    ...
    def execute(self, context):
        chainCollection = context.scene.re_chain_toolpanel.chainCollection
        chainSettingsObj = bpy.data.objects.get(self.chainSettingObjectName,None)
        Sub_Collection = chainSettingsObj.users_collection[0]
        chainGroupList = []#List of lists of nodes
        chainGroupObjList = []
        for selectedObject in bpy.context.selected_objects:
        ...
        if chainGroupList != [] and chainSettingsObj != None:
            ...
            for index, nodeList in enumerate(chainGroupList):
                ...
                chainSubGroupObj = createEmpty(f"CHAIN_GROUP_{str(groupIndex).zfill(2)}_SUBGROUP_{currentSubGroupID}", [("TYPE","RE_CHAIN_SUBGROUP")],chainSettingsObj,Sub_Collection)
                ...
                for node in nodeList:
                    ...
                    Sub_Collection.objects.link(newNode)
                    ...
                    if angleLimitObj != None:
                        ...
                        Sub_Collection.objects.link(newAngleLimitObj)
                        ...
                        if angleLimitHelperObj != None:
                            ...
                            Sub_Collection.objects.link(newAngleLimitHelperObj)
            ...
            chainSubGroupObj.select_set(True)
...
class chain2GroupClspFlags(AndFlags):
    def __init__(self):
        self.flagDict = {
            "None": {"enabled": False, "andFlag": 0},
            "Hip_HJ_00__Hip_HJ_00": {"enabled": False, "andFlag": 1},
            "Spine_0__Spine_0": {"enabled": False, "andFlag": 2},
            "Spine_1__Spine_1": {"enabled": False, "andFlag": 4},
            "Spine_2__Spine_2": {"enabled": False, "andFlag": 8},
            "Neck_0__L_UpperArm": {"enabled": False, "andFlag": 16},
            "Neck_0__R_UpperArm": {"enabled": False, "andFlag": 32},
            "L_UpperArm__L_Forearm": {"enabled": False, "andFlag": 64},
            "R_UpperArm__R_Forearm": {"enabled": False, "andFlag": 128},
            "L_Forearm__L_Hand": {"enabled": False, "andFlag": 256},
            "R_Forearm__R_Hand": {"enabled": False, "andFlag": 512},
            "L_Hand__L_MiddleF1": {"enabled": False, "andFlag": 1024},
            "R_Hand__R_MiddleF1": {"enabled": False, "andFlag": 2048},
            "Neck_0__Head": {"enabled": False, "andFlag": 4096},
            "Head__Head": {"enabled": False, "andFlag": 8192},
            "L_Thigh__L_Knee": {"enabled": False, "andFlag": 16384},
            "R_Thigh__R_Knee": {"enabled": False, "andFlag": 32768},
            "L_Shin__L_Foot": {"enabled": False, "andFlag": 65536},
            "R_Shin__R_Foot": {"enabled": False, "andFlag": 131072},
            "L_Foot__L_Toe": {"enabled": False, "andFlag": 262144},
            "R_Foot__R_Toe": {"enabled": False, "andFlag": 524288},
        }


class WM_OT_SetClspFlags(Operator):
    bl_label = "Set Clsp Flag"
    bl_description = "Set chain group clsp flags"
    bl_idname = "re_chain.set_clsp_flags"
    bl_context = "objectmode"
    bl_options = {'UNDO', 'INTERNAL'}

    Hip_HJ_00__Hip_HJ_00: bpy.props.BoolProperty(
        name="Hip_HJ_00 > Hip_HJ_00",
        description="",
        default=False)
    Spine_0__Spine_0: bpy.props.BoolProperty(
        name="Spine_0 > Spine_0",
        description="",
        default=False)
    Spine_1__Spine_1: bpy.props.BoolProperty(
        name="Spine_1 > Spine_1",
        description="",
        default=False)
    Spine_2__Spine_2: bpy.props.BoolProperty(
        name="Spine_2 > Spine_2",
        description="",
        default=False)
    Neck_0__L_UpperArm: bpy.props.BoolProperty(
        name="Neck_0 > L_UpperArm",
        description="",
        default=False)
    Neck_0__R_UpperArm: bpy.props.BoolProperty(
        name="Neck_0 > R_UpperArm",
        description="",
        default=False)
    L_UpperArm__L_Forearm: bpy.props.BoolProperty(
        name="L_UpperArm > L_Forearm",
        description="",
        default=False)
    R_UpperArm__R_Forearm: bpy.props.BoolProperty(
        name="R_UpperArm > R_Forearm",
        description="",
        default=False)
    L_Forearm__L_Hand: bpy.props.BoolProperty(
        name="L_Forearm > L_Hand",
        description="",
        default=False)
    R_Forearm__R_Hand: bpy.props.BoolProperty(
        name="R_Forearm > R_Hand",
        description="",
        default=False)
    L_Hand__L_MiddleF1: bpy.props.BoolProperty(
        name="L_Hand > L_MiddleF1",
        description="",
        default=False)
    R_Hand__R_MiddleF1: bpy.props.BoolProperty(
        name="R_Hand > R_MiddleF1",
        description="",
        default=False)
    Neck_0__Head: bpy.props.BoolProperty(
        name="Neck_0 > Head",
        description="",
        default=False)
    Head__Head: bpy.props.BoolProperty(
        name="Head > Head",
        description="",
        default=False)
    L_Thigh__L_Knee: bpy.props.BoolProperty(
        name="L_Thigh > L_Knee",
        description="",
        default=False)
    R_Thigh__R_Knee: bpy.props.BoolProperty(
        name="R_Thigh > R_Knee",
        description="",
        default=False)
    L_Shin__L_Foot: bpy.props.BoolProperty(
        name="L_Shin > L_Foot",
        description="",
        default=False)
    R_Shin__R_Foot: bpy.props.BoolProperty(
        name="R_Shin > R_Foot",
        description="",
        default=False)
    L_Foot__L_Toe: bpy.props.BoolProperty(
        name="L_Foot > L_Toe",
        description="",
        default=False)
    R_Foot__R_Toe: bpy.props.BoolProperty(
        name="R_Foot > R_Toe",
        description="",
        default=False)
    ...
    def draw(self, context):
        ...
        layout.label(text="Common clsp flags")
        layout.prop(self, "Hip_HJ_00__Hip_HJ_00")
        layout.prop(self, "Spine_0__Spine_0")
        layout.prop(self, "Spine_1__Spine_1")
        layout.prop(self, "Spine_2__Spine_2")
        layout.prop(self, "Neck_0__L_UpperArm")
        layout.prop(self, "Neck_0__R_UpperArm")
        layout.prop(self, "L_UpperArm__L_Forearm")
        layout.prop(self, "R_UpperArm__R_Forearm")
        layout.prop(self, "L_Forearm__L_Hand")
        layout.prop(self, "R_Forearm__R_Hand")
        layout.prop(self, "L_Hand__L_MiddleF1")
        layout.prop(self, "R_Hand__R_MiddleF1")
        layout.prop(self, "Neck_0__Head")
        layout.prop(self, "Head__Head")
        layout.prop(self, "L_Thigh__L_Knee")
        layout.prop(self, "R_Thigh__R_Knee")
        layout.prop(self, "L_Shin__L_Foot")
        layout.prop(self, "R_Shin__R_Foot")
        layout.prop(self, "L_Foot__L_Toe")
        layout.prop(self, "R_Foot__R_Toe")

    @classmethod
    def poll(self, context):
        return context.active_object is not None and (
                    context.active_object.get("TYPE") == "RE_CHAIN_CHAINGROUP")

    def execute(self, context):
        isChain2 = True

        flagClass = chain2GroupClspFlags()
        ...
        if activeObject != None:
            ...
            activeObject.re_chain_chaingroup.clspFlags0 = bitFlag
            ...
            for selectedObject in bpy.context.selected_objects:
                ...
                if selectedObjectType == activeObjectType:
                    if selectedObjectType == "RE_CHAIN_CHAINGROUP":
                        selectedObject.re_chain_chaingroup.clspFlags0 = bitFlag

            self.report({"INFO"}, "Set clsp flags.")
            tag_redraw(bpy.context)
        return {'FINISHED'}
    ...
    def invoke(self, context, event):
        ...
        flagClass.setFlagsFromInt(context.active_object.re_chain_chaingroup.clspFlags0)
        for key in flagClass.flagDict:
            if hasattr(self, key):
                # print(key)
