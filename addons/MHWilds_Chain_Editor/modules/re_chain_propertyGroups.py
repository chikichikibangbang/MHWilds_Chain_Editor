...
def filterChainCollection(self, collection):
    ...
    return True if ((collection.get("~TYPE") == "RE_CHAIN_COLLECTION") and (".chain" in collection.name)) else False
...
class chainToolPanelPropertyGroup(bpy.types.PropertyGroup):
	...
	def getChainPresets(self,context):
	...
	posebonealignAxis: EnumProperty(
		name="",
		...
		default = "Z")
	chainPresets: EnumProperty(
		name="",
		description="Set preset to be used by Apply Chain Preset button",
		items=getChainPresets
	)
